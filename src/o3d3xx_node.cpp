...
class O3D3xxNode {
  public:
    O3D3xxNode()
        ...
        //-----------------------------------------
        this->cam_ = std::make_shared<o3d3xx::Camera>(camera_ip, xmlrpc_port, password);
        ...
        // once.
        this->fg_ = std::make_shared<o3d3xx::FrameGrabber>(this->cam_, o3d3xx::IMG_UVEC);

        //----------------------
        // Published topics
        //----------------------
        this->cloud_pub_ = nh.advertise<pcl::PointCloud<o3d3xx::PointT>>("cloud", 1);
        ...
        // work.
        this->uvec_pub_ = nh.advertise<sensor_msgs::Image>("unit_vectors", 1, true);
        ...
        // Advertised services
        //----------------------
        this->version_srv_ = nh.advertiseService<o3d3xx::GetVersion::Request, o3d3xx::GetVersion::Response>(
            "GetVersion", std::bind(&O3D3xxNode::GetVersion, this, std::placeholders::_1, std::placeholders::_2));

        this->dump_srv_ =
            nh.advertiseService<o3d3xx::Dump::Request, o3d3xx::Dump::Response>("Dump", std::bind(&O3D3xxNode::Dump, this, std::placeholders::_1, std::placeholders::_2));

        this->config_srv_ =
            nh.advertiseService<o3d3xx::Config::Request, o3d3xx::Config::Response>("Config", std::bind(&O3D3xxNode::Config, this, std::placeholders::_1, std::placeholders::_2));

        this->rm_srv_ = nh.advertiseService<o3d3xx::Rm::Request, o3d3xx::Rm::Response>("Rm", std::bind(&O3D3xxNode::Rm, this, std::placeholders::_1, std::placeholders::_2));

        this->trigger_srv_ = nh.advertiseService<o3d3xx::Trigger::Request, o3d3xx::Trigger::Response>(
            "Trigger", std::bind(&O3D3xxNode::Trigger, this, std::placeholders::_1, std::placeholders::_2));
        this->crop_srv_ =
            nh.advertiseService<o3d3xx::Crop::Request, o3d3xx::Crop::Response>("Crop", std::bind(&O3D3xxNode::CropCB, this, std::placeholders::_1, std::placeholders::_2));
    }
    ...
    void Run() {
        ...
        pcl::PointCloud<o3d3xx::PointT>::Ptr cloud(new pcl::PointCloud<o3d3xx::PointT>());
        ...
        pcl::VoxelGrid<o3d3xx::PointT> vsor;
        vsor.setLeafSize(0.01f, 0.01f, 0.01f);
        ...
        while (ros::ok()) {
            ...
            if (!this->fg_->WaitForFrame(buff.get(), this->timeout_millis_)) {
                ...
                if ((ros::Time::now() - last_frame).toSec() > this->timeout_tolerance_secs_) {
                    ...
                    if (got_uvec) {
                        ...
                        this->fg_.reset(new o3d3xx::FrameGrabber(this->cam_, o3d3xx::IMG_UVEC));
                    }
                    fg_lock.unlock();
            ...
            if (!got_uvec) {
                sensor_msgs::ImagePtr uvec_msg = cv_bridge::CvImage(optical_head, sensor_msgs::image_encodings::TYPE_32FC3, buff->UnitVectors()).toImageMsg();
                this->uvec_pub_.publish(uvec_msg);

                ROS_INFO("Got unit vectors, restarting framegrabber with mask: %d", (int)this->schema_mask_);
    ...
    bool GetVersion(o3d3xx::GetVersion::Request &req, o3d3xx::GetVersion::Response &res) {
        ...
        ss << O3D3XX_LIBRARY_NAME << ": " << major << "." << minor << "." << patch;
    ...
    bool CropCB(o3d3xx::Crop::Request &req, o3d3xx::Crop::Response &res) {
